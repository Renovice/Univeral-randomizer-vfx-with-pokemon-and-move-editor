...
public class NDSRom {
    ...
    public void setOverlayRamSize(int number, int newSize) {
        if (number >= 0 && number < arm9overlays.length) {
            NDSY9Entry entry = arm9overlays[number];
            int oldRamSize = entry.ram_size;
            int oldStaticStart = entry.static_start;
            int oldStaticEnd = entry.static_end;
            entry.ram_size = newSize;
            if (oldStaticStart != 0 || oldStaticEnd != 0) {
                int oldStaticLength = oldStaticEnd - oldStaticStart;
                if (oldStaticLength == oldRamSize) {
                    entry.static_end = oldStaticStart + newSize;
                }
            }
        }
    }
